...
入力 JSON はクリーニング済み字幕と enrichment（outline_hints, entities_by_segment, numeric_facts, must_cover, uncertain_spans, instructions）です。
...
【手順】
1. outline_hints・must_cover・entities_by_segment・numeric_facts を把握
...
第1パスで生成された要約ドラフトを、reference（numeric_facts / must_cover / uncertain_spans）と照合し、誤りのみ修正した JSON を返します。
