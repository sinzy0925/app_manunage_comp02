...
NUMBER_FACT_RE = re.compile(
    ...
    r"\d{4}年|"
    r"\d{1,2}月(?:\d{1,2}日)?|"
    r"\d{1,4}(?:[,.]\d+)?(?:%|km|m|発|人|台|両|体|機|個|件|回|ドル|万)?"
    r")"
    r"(.{0,28})"
)

UNIT_OR_SCALE_RE = re.compile(
    r"(?:%|km|m|発|人|台|両|体|機|個|件|回|ドル|万|億|年|月|日|時間)"
)
...
- segment_summaries は [{index, bullets[]}] 配列。index は segments[].index（0始まり）"""
...
def extract_payload_range_facts(text: str, segment_index: int) -> list[dict]:
    """積載/射程の km 表現を高信号ファクトとして抽出（kg 混同防止）。"""
    facts: list[dict] = []
    seen: set[str] = set()
    for match in PAYLOAD_RANGE_RE.finditer(text):
...
def extract_date_facts(text: str, segment_index: int) -> list[dict]:
    """完全な日付（YYYY年M月D日）を高信号ファクトとして抽出。"""
    facts: list[dict] = []
    seen: set[str] = set()
...
def extract_numeric_facts(text: str, segment_index: int, limit: int = 20) -> list[dict]:
    ...
    for date_fact in extract_date_facts(text, segment_index):
        key = str(date_fact.get("value", "")).casefold()
        if key not in seen_ctx:
            seen_ctx.add(key)
            facts.append(date_fact)
    for range_fact in extract_payload_range_facts(text, segment_index):
        key = str(range_fact.get("value", "")).casefold()
        if key not in seen_ctx:
            seen_ctx.add(key)
            facts.append(range_fact)
    for inv_match in INVENTORY_RE.finditer(text):
        value = inv_match.group(1) + "体"
        context = text[max(0, inv_match.start() - 12) : min(len(text), inv_match.end() + 24)].strip()
    ...
    for match in NUMBER_FACT_RE.finditer(text):
        ...
        facts.append(normalize_numeric_fact(fact))
...
def assign_fact_ids(facts: list[dict]) -> list[dict]:
    for idx, fact in enumerate(facts):
        seg = int(fact.get("segment", 0))
        fact["id"] = f"nf_{seg:02d}_{idx:03d}"
    return facts


def build_must_cover(numeric_facts: list[dict], limit: int = 25) -> list[dict]:
    """高信号ファクトの優先リスト。完全日付は必ず含める。"""
    date_facts = [f for f in numeric_facts if f.get("kind") == "date" or FULL_DATE_RE.search(str(f.get("value", "")))]
    date_keys = {str(f.get("value")) for f in date_facts}
    others = [f for f in numeric_facts if str(f.get("value")) not in date_keys]
    ranked = date_facts + sorted(others, key=lambda f: int(f.get("score", 0)), reverse=True)
        ...
        out.append(
            {
                ...
                "quote": fact.get("quote", fact.get("context")),
            }
        )
        if len(out) >= limit:
            break
